...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(sc.textFile(WikipediaData.filePath).map(l => WikipediaData.parse(l)).collect())
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(a => a.mentionsLanguage(lang))
      .aggregate(0) (
        (count, _) => {
          count + 1
        },
